...
datacsv <- read_csv("doc_name_pcci.csv")
View(datacsv)
...
for(i in 1:nrow(data2) ){       # for-loop over rows
 # data2[i, ] <- data2[i, ] - 100
  ...
  datanombre=data2[i, 2]$`Apellidos / Nombres`
  ...
  #data <- read_html(url)
  #pg <- read_html(data)
  tryCatch(data %>% html_nodes(xpath = '//*[@id="inmuebledesc"]/div[1]/div[2]') %>% xml_attr("value"),
           error = function(e) { 
             print("Error in get html, Timed out 10 seconds, we go on ! i =")
             print(i)
           } )
  ...
  head(personasdf)
  if (i %% 500 == 0){
    write_csv(personasdf, ("contact_data_pcci.csv"))
  }
  
}
write_csv(personasdf, ("contact_data_pcci.csv.csv"))
View(personasdf)
