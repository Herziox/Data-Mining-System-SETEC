...
def save_info(id_bot,doc_name,data,columns,tds,tde,page,start_page,final_page):
    ...
    df.to_csv(doc_name+str(id_bot)+'.csv')
...
#TRY TO LOAD PAGE
  while(try_load):
    ...
    try:
      ...
      ## URL
      driver.get('http://portal.trabajo.gob.ec/setec-portal-web/pages/catalogoCualifacion.jsf')
      print('It works')

      ## SELECT MODULE
      driver.implicitly_wait(5)
      cmb_module = driver.find_element(By.LINK_TEXT, 'CNC para Organismos Evaluadores de la Conformidad')
      ...
      ## SELECT FILTER
      driver.implicitly_wait(5)
      cmb_seleccione_filtro = driver.find_element(By.ID, 'j_idt24:formCatalogoOEC:cmbCampoOEC:cmbCampoOEC_label')
      cmb_seleccione_filtro.click()

      ## SELECT OPTION
      driver.implicitly_wait(5)
      cmb_item_nombre_curso = driver.find_element(By.ID, 'j_idt24:formCatalogoOEC:cmbCampoOEC:cmbCampoOEC_4')
      cmb_item_nombre_curso.click()

      ## SEARCH QUERY
      driver.implicitly_wait(5)
      btn_buscar = driver.find_element(By.ID, 'j_idt24:formCatalogoOEC:j_idt78')
      btn_buscar.click()

      #GET LAST PAGE
      btn_control = driver.find_element(By.XPATH,'//*[@id="j_idt24:formCatalogoOEC:tblDatos_paginator_bottom"]/a[4]')
  ...
  while(go):  
    ...
    while(try_load):
      ...
      try:
        ...
        try:
          ...
          report_file.close()
          ## GET TABLE HEADER
          table_header = driver.find_element(By.ID,'j_idt24:formCatalogoOEC:tblDatos_head')
    ...
    while(page_number<page_checkpoint and page_number<final_page and not expired_session):
        try:
            ...
            btn_siguiente = driver.find_element(By.XPATH, '//*[@id="j_idt24:formCatalogoOEC:tblDatos_paginator_bottom"]/a[3]')

            if (page_checkpoint - page_number)>10:
              btn_siguiente = driver.find_element(By.XPATH,'//*[@id="j_idt24:formCatalogoOEC:tblDatos_paginator_bottom"]/a[4]')
    ...
    if (page_number >= final_page):
      ...
      while(condition):
        ...
        while(try_load):
          try:
            table_data = driver.find_element(By.ID,'j_idt24:formCatalogoOEC:tblDatos_data')
...
if __name__==('__main__'):
    ...
    for i in range(N_BOT_DRIVERS):
        ...
        browserThread.start()
        #bot(i,options,1,final_page,QUERY_INPUT)
